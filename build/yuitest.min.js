...
c=YUITest.Configs,d=$Y.one(document.body);b&&a.recordResultsForPage(a.$counter-1,b);if(a.$counter<c.pages.length){b=c.pages[a.$counter];$Y.log("Begin testing "+b,"warn");d.append($Y.Lang.sub('<div id="'+c.testee_id_tpt+'">Running tests on {page}...</div>',{counter:a.$counter,page:b}));a.updateTestee($Y.Lang.sub("{protocol}//{host}{page}",{protocol:window.location.protocol,host:window.location.host,page:b}));++a.$counter}else{d.append($Y.Lang.sub("<hr /><h3>All tests complete.</h3>"+c.result_tpt,{id:c.final_tally,
duration:(a.$tally.duration/1E3).toFixed(2),ignored:a.$tally.ignored,failed:a.$tally.failed,passed:a.$tally.passed,total:a.$tally.total}));a.$testee&&a.$testee.YUITest.Testee.$tester&&a.$testee.YUITest.Testee.$tester.close();a.$testee&&a.$testee.close();$Y.log("All tests completed","warn")}return a},updateTestee:function(b){var a=YUITest.TestSuite,c=YUITest.Configs;if(c.testee_type==="iframe"){var d="#"+c.testee_window_name,e=$Y.one(d);if(!e){$Y.one(document.body).append($Y.Lang.sub('<iframe id="{id}" name="{id}" frameborder="0"></iframe>',
{id:c.testee_window_name}));e=$Y.one(d).setStyles({width:"0",height:"0"})}e.set("src",b)}else a.$testee=window.open(b,c.testee_window_name);return a}};
