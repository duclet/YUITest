var YUITest={version:0.2,TestCases:{},loadYUI:function(b,a){if(a){var c=b;b=function(d){window.$Y=d;c.apply(this)}}YUI().use("node","console","test",b)}};
...
YUITest.TestSuite={$counter:0,$tally:{duration:0,ignored:0,failed:0,passed:0,total:0},$testee:null,initialize:function(){YUITest.loadYUI(function(){var b=YUITest.Configs.load();(new $Y.Console(b.global_logger_config)).render(b.global_logger);YUITest.TestSuite.runNextTestPage()},true)},logEvent:function(b){b.message.message.indexOf("Test suite")!==0&&$Y.log(b.message.message,b.message.category,YUITest.Configs.pages[YUITest.TestSuite.$counter-1])},recordResultsForPage:function(b,a){var c=YUITest.TestSuite,
...
c=YUITest.Configs,d=$Y.one(document.body);b&&a.recordResultsForPage(a.$counter-1,b);if(a.$counter<c.pages.length){b=c.pages[a.$counter];$Y.log("Begin testing "+b,"warn");d.append($Y.Lang.sub('<div id="'+c.testee_id_tpt+'">Running tests on {page}...</div>',{counter:a.$counter,page:b}));a.updateTestee($Y.Lang.sub("{protocol}//{host}{page}",{protocol:window.location.protocol,host:window.location.host,page:b}));++a.$counter}else{d.prepend($Y.Lang.sub((a.$tally.failed?'<h3 style="color: red;">Aww, {failed} tests failed.</h3>':
'<h3 style="color: green;">Congrats! No failed tests!</h3>')+c.result_tpt+"<hr />",{id:c.final_tally,duration:(a.$tally.duration/1E3).toFixed(2),ignored:a.$tally.ignored,failed:a.$tally.failed,passed:a.$tally.passed,total:a.$tally.total}));a.$testee&&a.$testee.YUITest.Testee.$tester&&a.$testee.YUITest.Testee.$tester.close();a.$testee&&a.$testee.close();$Y.log("All tests completed","warn")}return a},updateTestee:function(b){var a=YUITest.TestSuite,c=YUITest.Configs;if(c.testee_type==="iframe"){var d=
"#"+c.testee_window_name,e=$Y.one(d);if(!e){$Y.one(document.body).append($Y.Lang.sub('<iframe id="{id}" name="{id}" frameborder="0"></iframe>',{id:c.testee_window_name}));e=$Y.one(d).setStyles({width:"0",height:"0"})}e.set("src",b)}else a.$testee=window.open(b,c.testee_window_name);return a}};
YUITest.Testee={$tester:null,initialize:function(){YUITest.loadYUI(function(){var b=window==window.parent?window.opener:window.parent;if(b&&b.YUITest)window.$C=b;YUITest.Configs.load();YUITest.Testee.start()},true)},getTestCases:function(){var b=$Y.one(YUITest.Configs.test_cases_selector);b=b?b.get("innerHTML").split(","):[];$Y.Array.each(b,function(a,c){b[c]=$Y.Lang.trim(a)});return b},start:function(){var b=YUITest.Testee,a=YUITest.Configs,c=$Y.Lang.sub(a.tester_url_tpt,{protocol:window.location.protocol,
host:window.location.host});if(a.tester_type==="iframe"){var d="#"+a.tester_window_name,e=$Y.one(d);if(!e){$Y.one(document.body).append($Y.Lang.sub('<iframe id="{id}" name="{id}"></iframe>',{id:a.tester_window_name}));e=$Y.one(d).setStyles({height:"400px",width:"100%"})}e.set("src",c)}else b.$tester=window.open(c,a.tester_window_name);return b}};
